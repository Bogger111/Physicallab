...
async def data_collection_status():
    """Expose only whether the optional contribution UI should be shown."""
...
@app.post("/api/data-collection/sessions")
async def create_data_collection_session(
    ...
    """Store a de-identified original sheet only after explicit consent."""
...
@app.post("/api/data-collection/sessions/{session_id}/commit")
async def commit_data_collection_session(session_id: str, req: CollectionCommitRequest):
    """Save confirmed report data as stable field labels for one raw image."""
    from app.data_collection import (
...
@app.delete("/api/data-collection/sessions/{session_id}")
async def delete_data_collection_session(session_id: str):
    """Allow withdrawal before or after confirmation."""
    from app.data_collection import (
        CollectionNotFoundError,
        CollectionValidationError,
        collection_enabled,
